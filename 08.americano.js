...
//? Constraints: 0 < X < 11. N < 12

//? Output Format: Imprima na tela a posição em que está o jogodar que foi sorteado para ser o goleiro.

//? Sample Input 0: 1 3 2 1.
...
const solucao = (numeros) => {
    let soma = numeros.reduce((soma, i) => soma + i),
        jogador = soma % numeros.length == 0 ? numeros.length : Math.floor(soma % numeros.length);
    console.log(jogador);
};
...
solucao([1, 2]);
solucao([2, 3, 4, 5, 6]);
solucao([2, 2, 2, 2, 2, 2, 2, 2, 2, 2, 2, 2]);
solucao([1, 3, 4, 2, 1]);
solucao([1, 3, 4, 2, 3, 3, 4]);
solucao([1, 2, 1, 2]);
solucao([1, 2, 3, 4, 5, 1]);